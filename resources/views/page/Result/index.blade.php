            ...
            <div class="card-body">
                <form method="GET" action="{{ request()->fullUrlWithQuery }}" autocomplete="off">
                    ...
                    <button class="btn btn-primary btn-sm nextBtn btn-lg pull-right h-10 mr-2" type="submit"
                        formaction="{{ route('Result.index') }}">عرض</button>
                    <button class="btn btn-primary btn-sm nextBtn btn-lg pull-right h-10" type="submit"
                        formaction="{{ route('Result.create') }}">اضافة</button>
                </form>
            </div>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <div class="card-body">
                <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                    ...
                    <tbody>
                        @forelse ($Result as $AcademicWork)
                            @php
                                $link =
                                    'Result/create?college_id=' .
                            ...
                            <tr>
                                ...
                                <td><a class="w-100 h-100 d-inline-block"
                                        href="{{ $link }}">{{ $AcademicWork->final_exam_grade }}</a></td>
                                <td><a class="w-100 h-100 d-inline-block"
                                        href="{{ $link }}">{{ $AcademicWork->final_grade }}</a></td>
                                <td><a class="w-100 h-100 d-inline-block"
                                        href="{{ $link }}">{{ $AcademicWork->status }}</a></td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        ...
        function updateTableBody(data) {
            ...
            if (data.length === 0) {
                ...
                data.forEach(item => {
                    const link =
                        `Result/create?college_id=${item.college_id}&specialization_id=${item.specialization.id}&semester_num=${item.semester_num}&course_id=${item.course.id}&student_id=${item.student.id}`;
                    row = $(`<tr>`);
